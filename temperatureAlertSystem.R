#upstreamData, downstreamData, 
#parameter <- 'Turb_Inst'
#turbidityBaseline <- 40
#turbidityUnitIncrease <-  -0.6
#turbidityPercentIncrease <- 15
#parameter <- 'Wtemp_Inst'
#WQclass <- 6
...
# Turbidity analysis
turbidity <- function(upstreamData, downstreamData, parameter, turbidityBaseline, turbidityUnitIncrease, turbidityPercentIncrease){
  ...
  if(min(UP$upstream, na.rm = T) >= turbidityBaseline && min(DOWN$downstream, na.rm = T) >= turbidityBaseline){
    tidyverse_diff_rollstats <- together %>%
      tq_mutate(
        ...
        # FUN args
        threshold  = turbidityPercentIncrease,
        # tq_mutate args
    ...
    tidyverse_diff_rollstats <- together %>%
      tq_mutate(
        ...
        # FUN args
        threshold  = turbidityUnitIncrease,
        # tq_mutate args
...
dataScan <- function(upstreamData, downstreamData, WQclass, turbidityBaseline, turbidityUnitIncrease, turbidityPercentIncrease){
  ...
  return(turbidityResults)#tempResults)
